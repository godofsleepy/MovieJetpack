...
class DetailFilmViewModel(private val movieRepository: MovieRepository) : ViewModel() {
    ...
    fun getDetailFilm(id: Int): LiveData<DetailMovieResponse> =
        movieRepository.getDetailMovie(id.toString())

    fun getRelatedMovie(idGenre: String): LiveData<List<MovieResponse>> =
        movieRepository.getMovieByGenre(idGenre)
